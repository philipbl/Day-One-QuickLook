...
NSString* getDayOneData(NSURL* url, NSStringEncoding encoding);
...
NSData* processDayOne(NSURL* url)
{
    ...
    NSString* theData = getDayOneData(url, encoding);
...
NSString* getDayOneData(NSURL* url, NSStringEncoding encoding) {
    ...
    NSString *entryString = [NSString stringWithContentsOfFile:[url path] usedEncoding:&encoding error:nil];
    NSData* entryData = [entryString dataUsingEncoding:NSUTF8StringEncoding];
    
    NSString *error;
    NSPropertyListFormat format;
    NSDictionary* entry = [NSPropertyListSerialization propertyListFromData:entryData mutabilityOption:NSPropertyListImmutable format:&format errorDescription:&error];
    ...
    // Get entry text
    NSString* text = [entry objectForKey:@"Entry Text"];
    
    // Get if it is starred
    NSString* starred = [NSString stringWithFormat:@"%@", [entry objectForKey:@"Starred"]];
    NSString* starred_text = @"";
    
    
    //if ([starred isEqualToString:starred_text]) {
        //starred_text = @"<span class=\"starred\">STARRED</span>";
    //    NSLog(@"TEST");
    //}
    
    if([starred isEqualToString:@"1"]){
        starred_text = @"<span class=\"starred\">&#x2605;</span>";
    }
    
    // Get time
    NSDate* time = [entry objectForKey:@"Creation Date"];
    
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"MMMM dd, yyyy h:mm a"];
    
    NSString* time_text = [NSString stringWithFormat:@"<span class=\"time\">%@</span>", [formatter stringFromDate:time]];
    [formatter release];
    
    // Get location
    NSDictionary* location = [entry objectForKey:@"Location"];
    NSString* location_text = @"";
    
    if (location != nil) {
        location_text = [NSString stringWithFormat:@"&bull;<span class=\"location\">%@, %@, %@, %@</span>",
                         [location objectForKey:@"Place Name"],
                         [location objectForKey:@"Locality"],
                         [location objectForKey:@"Administrative Area"],
                         [location objectForKey:@"Country"]];
    }
    
    // Get weather
    NSDictionary* weather = [entry objectForKey:@"Weather"];
    NSString* weather_text = @"";
    
    if (weather != nil) {
        weather_text = [NSString stringWithFormat:@"&bull;<span class=\"weather\">%@&deg; %@</span>",
                        [weather objectForKey:@"Fahrenheit"],
    ...
    if (tags != nil && [tags count] > 0) {
        tag_text = [NSString stringWithFormat:@"&bull;<span class=\"tags\">%@</span>",
                    [tags componentsJoinedByString:@"<span class=\"bullet\">&bull;</span>"]];
    }
    
    NSString* style = @"<style>.meta-data{font-size:10px;color:#c1c1c1;font-weight:500;margin-bottom:10px}.meta-data span{margin:0 8px}.meta-data span:first-child{margin-left:0}.time,.tags{color:#48b7dc}.location,.weather{color:#afa9af}.bullet{margin:0 8px!important;color:#c1c1c1}.starred{margin:0!important;color:#f3b839}</style>";
    
    NSString* final_text = @"%@\n<div class=\"meta-data\">%@%@%@%@%@</div>\n%@";
    
    return [NSString stringWithFormat:final_text, style, starred_text, time_text, location_text, weather_text, tag_text, text];
